...
export default function ProfessionalsBeheerPage() {
  ...
  async function ladenProfessionals() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Professionals konden niet worden geladen.");
      }
      ...
      setFout(
        error instanceof Error
          ? error.message
          : "Professionals konden niet worden geladen."
      );
    } finally {
      setLaden(false);
    }
  ...
  async function professionalVerwijderen(professional: Professional) {
    const naam =
      ...
      `${professional.voornaam || ""} ${professional.achternaam || ""}`.trim() ||
      professional.email ||
      "deze professional";

    const akkoord = window.confirm(
      `Weet je zeker dat je ${naam} wilt verwijderen? Dit kan alleen als er geen boekingshistorie is.`
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.details || data.error || "Verwijderen mislukt.");
      }

      setProfessionals((huidig) =>
        huidig.filter((item) => item.id !== professional.id)
      );
    } catch (error) {
      setFout(
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600">
              Verwijderen is alleen mogelijk als er geen boekingshistorie bestaat.
            </p>
          </div>
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="border-b bg-gray-50">
                <tr>
                  ...
                  <th className="px-5 py-4 font-semibold text-gray-700">Actie</th>
                </tr>
              </thead>
                ...
                {professionals.length === 0 ? (
                  ...
                  professionals.map((professional) => (
                    <tr key={professional.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-4 text-gray-600">
                        {professional.postcode || "-"} {professional.woonplaats || ""}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex flex-wrap gap-2">
                          ...
                          >
                            {professional.geverifieerd ? "Geverifieerd" : "Controle nodig"}
                          </span>
                          <span
                            className={`rounded-full px-3 py-1 text-xs font-semibold ${
                              professional.actief
                          ...
                          >
                            {professional.actief ? "Actief" : "Niet actief"}
                          </span>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <button
                          type="button"
                          disabled={bezigId === professional.id}
                          onClick={() => professionalVerwijderen(professional)}
                        ...
                        >
                          {bezigId === professional.id ? "Verwijderen..." : "Verwijderen"}
                        </button>
                      </td>
                    </tr>
                  ))
