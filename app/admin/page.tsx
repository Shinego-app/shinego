...
export default function AdminPage() {
  ...
  function volledigeNaam(voornaam?: string, achternaam?: string) {
  ...
  async function professionalKoppelen(
    ...
    try {
      const response = await fetch("/api/admin", {
        ...
        body: JSON.stringify({ booking_id, professional_id }),
      });
  ...
  async function bookingStatusBijwerken(
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(
          data.details || data.error || "Boekingstatus bijwerken mislukt"
        );
      }
      ...
      setFout(
        error instanceof Error
          ? error.message
          : "Boekingstatus bijwerken mislukt"
      );
    }
  }
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <header className="border-b bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          <div>
            ...
            <div className="text-sm text-gray-500">Beheeromgeving</div>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">ShineGo beheer</h1>
          <p className="mt-2 text-gray-600">Overzicht van boekingen.</p>
        </div>
          ...
          <>
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border bg-white p-6 shadow-sm">
                <div className="text-sm font-medium text-gray-500">Boekingen</div>
              ...
              <div className="rounded-2xl border bg-white p-6 shadow-sm">
                <div className="text-sm font-medium text-gray-500">Actief</div>
                <div className="mt-2 text-3xl font-bold text-green-600">
                  {actieveProfessionals}
                </div>
              </div>
              <div className="rounded-2xl border bg-white p-6 shadow-sm">
                <div className="text-sm font-medium text-gray-500">Geverifieerd</div>
            ...
            <div className="mt-10">
              <div className="mb-5">
                <h2 className="text-2xl font-bold text-gray-900">Recente boekingen</h2>
              ...
              <div className="overflow-x-auto rounded-2xl border bg-white shadow-sm">
                <table className="min-w-full text-left text-sm">
                  <thead className="border-b bg-gray-50">
                    <tr>
                      <th className="px-5 py-4 font-semibold text-gray-700">Datum</th>
                      <th className="px-5 py-4 font-semibold text-gray-700">Klant</th>
                      <th className="px-5 py-4 font-semibold text-gray-700">Contact</th>
                      <th className="px-5 py-4 font-semibold text-gray-700">Adres</th>
                      <th className="px-5 py-4 font-semibold text-gray-700">Prijs</th>
                      <th className="px-5 py-4 font-semibold text-gray-700">Status</th>
                      <th className="px-5 py-4 font-semibold text-gray-700">Acties</th>
                      <th className="px-5 py-4 font-semibold text-gray-700">Toewijzing</th>
                    </tr>
                  </thead>
                      ...
                      boekingen.slice(0, 20).map((boeking) => (
                        <tr key={boeking.id} className="hover:bg-gray-50">
                          ...
                          <td className="whitespace-nowrap px-5 py-4 font-medium text-gray-900">
                            {volledigeNaam(boeking.voornaam, boeking.achternaam)}
                          </td>
                          ...
                          <td className="px-5 py-4 text-gray-600">
                            <div>{boeking.straat || "-"} {boeking.huisnummer || ""}</div>
                            <div>{boeking.postcode || ""} {boeking.plaats || ""}</div>
                          ...
                          <td className="whitespace-nowrap px-5 py-4 font-semibold text-gray-900">
                            ...
                            <div className="mt-2 text-xs font-normal text-gray-600">
                              Kozijnen toeslag: {boeking.kozijnen ? "ja" : "nee"}
                            </div>
                            <div className="text-xs font-normal text-gray-600">
                              Toeslag prijs: €{boeking.kozijnen ? Number(boeking.kozijnen_toeslag ?? 0).toFixed(2).replace(".", ",") : "0,00"}
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-4">
                            ...
                            <div className="mt-2 text-xs text-gray-600">
                              Betaald: {boeking.betaald ? "Ja" : "Nee"}
                            </div>
                            {boeking.annuleringsreden && (
                              <div className="mt-1 text-xs text-gray-600">
                                Reden: {boeking.annuleringsreden}
                              </div>
                            )}
                          </td>
                          ...
                          <td className="px-5 py-4">
                            <div className="flex w-40 flex-col gap-2">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const reden = window.prompt(
                                    "Reden annulering door professional:"
                                  );
                                  if (reden === null) return;
                                  bookingStatusBijwerken(
                                    boeking.id,
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const bewijs = window.prompt(
                                    "Bewijs dat de klant niet thuis was:"
                                  );
                                  if (!bewijs?.trim()) return;
                                  bookingStatusBijwerken(
                                    boeking.id,
                                    "geannuleerd",
                                    "Klant niet thuis",
                                ...
                                className="rounded-lg bg-yellow-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"
                              >
                                Klant niet thuis
                              </button>
                          ...
                          <td className="px-5 py-4">
                            <select
                              value={boeking.professional_id || ""}
                              disabled={boeking.status === "geannuleerd"}
                              onChange={(event) => {
                                if (event.target.value) {
                                  professionalKoppelen(
                                    boeking.id,
                                    event.target.value
                                  );
                                }
