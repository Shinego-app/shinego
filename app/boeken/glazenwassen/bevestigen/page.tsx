...
export default function BevestigenPage() {
  ...
  const bedrijf = klus?.type === "bedrijf" || klus?.woningtype === "bedrijfspand";
  const alleenBinnen = Boolean(klus?.alleenBinnen) || klus?.type === "binnen";
  const binnenkant = Boolean(klus?.binnenkant) && !alleenBinnen;
      ...
      <section className="mt-5 rounded-[28px] border border-[#d5e9f8] bg-white/90 px-5 py-6 shadow-[0_18px_55px_rgba(40,93,140,.12)] sm:px-7 sm:py-7">
        ...
        {!klus || !details || !prijs || !klant ? <p className="mt-8 text-[#6d89a4]">Boekingsgegevens laden...</p> : <>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-[#d5e9f8] bg-[#f8fcff] p-4"><h2 className="text-sm font-extrabold">Jouw gegevens</h2><dl className="mt-3 space-y-2 text-sm text-[#5f7e9c]"><div className="flex justify-between gap-3"><dt>Naam</dt><dd className="text-right font-semibold text-[#123c70]">{klant.voornaam} {klant.achternaam}</dd></div><div className="flex justify-between gap-3"><dt>E-mail</dt><dd className="text-right font-semibold text-[#123c70]">{klant.email}</dd></div><div className="flex justify-between gap-3"><dt>Telefoon</dt><dd className="text-right font-semibold text-[#123c70]">{klant.telefoon || "Niet opgegeven"}</dd></div><div className="flex justify-between gap-3"><dt>Adres</dt><dd className="text-right font-semibold text-[#123c70]">{klant.straat} {klant.huisnummer}{klant.toevoeging ? ` ${klant.toevoeging}` : ""}<br />{klant.postcode} {klant.plaats}</dd></div></dl></div>
            <div className="rounded-2xl border border-[#d5e9f8] bg-[#f8fcff] p-4"><h2 className="text-sm font-extrabold">Opdracht</h2><dl className="mt-3 space-y-2 text-sm text-[#5f7e9c]"><div className="flex justify-between gap-3"><dt>Dienst</dt><dd className="text-right font-semibold text-[#123c70]">{soort}</dd></div><div className="flex justify-between gap-3"><dt>Type woning/pand</dt><dd className="text-right font-semibold text-[#123c70]">{klus.woningtype || "Niet opgegeven"}</dd></div>{bedrijf ? <>
              <div className="flex justify-between gap-3"><dt>Reiniging</dt><dd className="text-right font-semibold text-[#123c70]">{reiniging}</dd></div>
              {!alleenBinnen&&<div className="flex justify-between gap-3"><dt>Glas buiten</dt><dd className="text-right font-semibold text-[#123c70]">{klus.glasOppervlak || "Niet opgegeven"}</dd></div>}
          ...
          {klus.frequentie !== "eenmalig" && <div className="mt-4 rounded-2xl border border-[#b9ddf8] bg-[#f4faff] p-4 text-xs leading-5 text-[#4f708f]"><strong className="text-[#0b3d75]">Geen automatische afschrijving.</strong> Na iedere afgeronde beurt zet ShineGo de volgende afspraak volgens jouw gekozen frequentie klaar. Je ontvangt voor elke volgende beurt eerst een apart betaalverzoek.</div>}
          <div className="mt-5 space-y-3"><label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${akkoordVoorwaarden?"border-[#1683f8] bg-[#eef8ff]":"border-[#d5e9f8] bg-white"}`}><input type="checkbox" checked={akkoordVoorwaarden} onChange={(e)=>setAkkoordVoorwaarden(e.target.checked)} className="mt-1" /><span className="text-xs leading-5 text-[#5f7e9c]">Ik ga akkoord met de <a href="/voorwaarden" target="_blank" className="font-bold text-[#1683f8] underline">algemene voorwaarden</a> en het annuleringsbeleid. Informatie over het wettelijke herroepingsrecht en de online mogelijkheid om te herroepen staat op <a href="/herroepen" target="_blank" className="font-bold text-[#1683f8] underline">Overeenkomst herroepen</a>.</span></label><label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${akkoordStartBedenktijd?"border-[#1683f8] bg-[#eef8ff]":"border-[#d5e9f8] bg-white"}`}><input type="checkbox" checked={akkoordStartBedenktijd} onChange={(e)=>setAkkoordStartBedenktijd(e.target.checked)} className="mt-1" /><span className="text-xs leading-5 text-[#5f7e9c]">Ik verzoek ShineGo om de dienstverlening, indien nodig, binnen de wettelijke bedenktijd te laten starten.</span></label></div>
          {fout && (
