  ...
  const prijs = useMemo(() => {
    ...
    const bedrijfsPrijs =
      gegevens.glasOppervlak === "0-15"
        ? gegevens.telescoop ? 69 : 49
        : gegevens.glasOppervlak === "16-30"
          ? gegevens.telescoop ? 99 : 69
          : gegevens.glasOppervlak === "31-50"
            ? gegevens.telescoop ? 149 : 109
            : gegevens.glasOppervlak === "51-100"
              ? gegevens.telescoop ? 259 : 189
              : gegevens.glasOppervlak === "101-200"
                ? gegevens.telescoop ? 469 : 349
                : gegevens.glasOppervlak === "201-500"
                  ? gegevens.telescoop ? 999 : 749
    ...
    const kozijnenToeslag = details.kozijnen
      ? 9.95 + Math.max(0, gegevens.ramen - 10)
      : 0;
    ...
    <main className="min-h-screen bg-gray-50">
      <header className="border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6 sm:py-5">
          ...
          <a href="/boeken/glazenwassen/details" className="text-sm font-medium text-gray-600 hover:text-blue-600 sm:text-base">← Terug</a>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
        <div className="mb-7 sm:mb-10">
          ...
          <div className="h-2 rounded-full bg-gray-200"><div className="h-2 w-full rounded-full bg-blue-600" /></div>
        </div>
        ...
        <div className="mb-7 text-center sm:mb-10">
          ...
          <p className="mt-2 text-base text-gray-600 sm:mt-3 sm:text-lg">Op basis van de gegevens van jouw opdracht.</p>
        </div>

        <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-lg">
          <div className="bg-blue-600 px-5 py-7 text-center text-white sm:px-8 sm:py-10">
            <p className="text-xs font-semibold uppercase tracking-wide text-blue-100 sm:text-sm">Geschatte totaalprijs</p>
            ...
            <p className="mt-2 text-sm text-blue-100 sm:mt-3 sm:text-base">Voor glazenwassen buitenzijde</p>
          </div>
          ...
          <div className="p-5 sm:p-8">
            ...
            <div className="mt-5 space-y-4 sm:mt-6">
              <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4">
                <span className="text-gray-600">{gegevens.woningtype === "bedrijfspand" ? "Zakelijke glasprijs" : "Basisprijs"}</span>
                <span className="shrink-0 text-right font-semibold text-gray-900">
                  {offerteOpMaat ? "Offerte op maat" : `€${(gegevens.woningtype === "bedrijfspand" ? prijs.bedrijfsPrijs : prijs.basisprijs).toFixed(2).replace(".", ",")}`}
                </span>
              </div>

              <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4">
                <span className="text-gray-600">{gegevens.woningtype === "bedrijfspand" ? gegevens.glasOppervlak : `${gegevens.ramen} ramen`}</span>
                <span className="shrink-0 text-right font-semibold text-gray-900">
              ...
              {prijs.verdiepingToeslag > 0 && (
                <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4">
                  ...
                  <span className="shrink-0 font-semibold text-gray-900">€{prijs.verdiepingToeslag.toFixed(2).replace(".", ",")}</span>
                </div>
              )}
              ...
              {prijs.bereikToeslag > 0 && (
                <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4">
                  ...
                  <span className="shrink-0 font-semibold text-gray-900">€{prijs.bereikToeslag.toFixed(2).replace(".", ",")}</span>
                </div>
              )}
              ...
              {prijs.kozijnenToeslag > 0 && (
                <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4">
                  ...
                  <span className="shrink-0 font-semibold text-gray-900">€{prijs.kozijnenToeslag.toFixed(2).replace(".", ",")}</span>
                </div>
              )}

              {prijs.kortingPercentage > 0 && (
                <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4">
                  <span className="text-gray-600">Abonnementskorting ({Math.round(prijs.kortingPercentage * 100)}%)</span>
                  <span className="shrink-0 font-semibold text-green-600">- €{prijs.kortingBedrag.toFixed(2).replace(".", ",")}</span>
                </div>
              )}
              ...
              <div className="flex items-center justify-between gap-4 pt-2 text-lg sm:text-xl">
                ...
                <span className="font-bold text-blue-600">{offerteOpMaat ? "Offerte" : `€${prijs.totaal.toFixed(2).replace(".", ",")}`}</span>
              </div>
            </div>
          </div>
        </div>
