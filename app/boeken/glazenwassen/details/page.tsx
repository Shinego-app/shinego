...
export default function DetailsPage() {
  ...
  useEffect(() => {
    ...
    if (data.type === "telewash") {
      ...
      setKozijnen(true);
      setBereikbaar("nee");
    } else if (data.verdiepingen?.includes("4")) {
      setBereikbaar("nee");
    }
  ...
  const appartementBuiten = appartement && appartementToegang === "buiten";
  ...
  const derdeVerdiepingVerplicht = verdiepingen.includes("4");
  ...
  const kanVerder = Boolean(gegevens) &&
    ...
    (bedrijf
      ? glasOppervlak !== ""
      : appartement
        ? ramen > 0 && appartementToegang !== "" && (appartementToegang === "balkon" || verdiepingen.length === 1)
        : ramen > 0 && verdiepingen.length > 0);
      ...
      <section className="mx-auto mt-5 max-w-[860px] rounded-[24px] border border-[#d5e9f8] bg-white/95 px-5 py-6 shadow-[0_12px_35px_rgba(44,95,135,.08)] sm:px-7 sm:py-7 lg:mt-7 lg:px-9 lg:py-8">
        ...
        {!gegevens?<p className="mt-8 text-[#6d89a4]">Gegevens laden...</p>:<>
          ...
          <div className="mt-5 divide-y divide-[#e5f0f8] rounded-2xl border border-[#dcecf8] bg-[#fbfdff] px-4">
            ...
            <button type="button" disabled={telewash || derdeVerdiepingVerplicht} onClick={()=>setBereikbaar(bereikbaar==="ja"?"nee":"ja")} className={`flex w-full items-center justify-between py-4 text-left ${telewash||derdeVerdiepingVerplicht?"cursor-not-allowed":""}`}><div><div className="text-sm font-extrabold text-[#123c70]">Extra lastig bereikbaar?</div><div className="text-xs text-[#6d89a4]">{telewash?"Standaard bij hoog en groot glas":derdeVerdiepingVerplicht?"Automatisch bij de 3e verdieping":"Bijvoorbeeld boven een serre of schuin dak"}</div></div><span className={`relative h-7 w-12 rounded-full ${bereikbaar==="nee"?"bg-[#1683f8]":"bg-[#d5e5f2]"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow ${bereikbaar==="nee"?"left-6":"left-1"}`}/></span></button>
          </div>
