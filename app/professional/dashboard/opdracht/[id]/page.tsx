...
function zichtbareOpmerking(value: unknown) {
  return String(value || "")
    .split("\n")
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-50 to-slate-50 px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="overflow-hidden rounded-3xl bg-slate-950 text-white shadow-xl"><div className="px-6 py-7 sm:px-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold uppercase tracking-wider text-blue-400">ShineGo opdracht</p><h1 className="mt-2 min-w-0 text-3xl font-extrabold [overflow-wrap:anywhere]">{opdracht.voornaam} {opdracht.achternaam}</h1><p className="mt-2 text-slate-300">{opdracht.gewenste_datum || "Nog niet gepland"} · {opdracht.gewenste_tijd || "Nog niet gepland"}</p></div><span className="w-fit rounded-full bg-blue-500/15 px-4 py-2 text-sm font-bold capitalize text-blue-300 ring-1 ring-blue-400/20">{statusLabel}</span></div></div></div>
        ...
        <section className="mt-6 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm"><h2 className="text-xl font-bold text-slate-900">Acties</h2><p className="mt-1 text-sm text-slate-500">Starten en afronden worden door ShineGo geblokkeerd zolang de geplande datum en begintijd nog niet zijn bereikt.</p><div className="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          ...
          {opdracht.status === "afgerond" && opdracht.uitbetaald !== true && <div className="rounded-xl bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">Opdracht afgerond. Je vergoeding wordt automatisch meegenomen in de wekelijkse uitbetalingsronde.</div>}
...
function Info({ label, value }: { label: string; value: any }) {
  return <div className="flex min-w-0 items-start justify-between gap-4 border-b border-slate-100 pb-3 last:border-0 last:pb-0"><span className="min-w-0 shrink-0 text-slate-500">{label}</span><span className="min-w-0 break-words text-right font-semibold text-slate-900">{String(value)}</span></div>;
}
